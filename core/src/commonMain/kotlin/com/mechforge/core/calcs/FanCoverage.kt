 ...
 * practical question - how many fans of the size I am going to install does that take, and how
 * much margin does the installation carry? These functions answer that, and they are pure so the
...
object FanCoverage {
    ...
    /** Airflow installed by [count] fans of [capacityPerFan], or null when either is missing. */
    ...
    /** Installed capacity relative to the requirement, in percent, or null when it cannot form. */
    ...
    /**
     * What the stated installation says about itself: short capacity, too few fans to reach the
     * requirement, or a very large oversize. Empty when the installation covers the requirement
    ...
    fun warnings(
        ...
        if (required > 0.0 && installed < required) {
            ...
            add(
                "Installed fan capacity is short of the calculated requirement by " +
        ...
        if (margin != null && margin > 100.0) {
            add(
                "Installed capacity is more than double the requirement (+" + Fmt.n(margin, 0) +
